...
これまで学んだソートアルゴリズム（挿入ソート、および**第9回で学んだバブルソート**など）は、
**要素同士を比較**して順序を決めていました。第9回では挿入ソート・二分挿入ソート・バブルソートを比べましたが、
どれも比較ベースで、最速でも O(n log n) より速くはできません。
...
> **「比較ベースのソートの下限」とは？**
> 「2つを比べて並べる」やり方では、どんなに工夫しても **O(n log n) より速くできない** という"速さの限界ライン"のこと。
> 理由のイメージ: n 個の並び方は n! 通りあり、比較1回（はい/いいえ）では候補を半分にしか絞れない。
> n! 通りを半分ずつ絞って1つに特定するには最低でも log₂(n!) ≒ n log n 回の比較が必要 → これが超えられない壁。
> 挿入ソートもバブルソートも「比べて並べる」ので、この壁の中にいる。

分布数えソート（Counting Sort）は**比較を一切使いません**。
代わりに、**各値が何回出現するかを数える**ことでソートします。
だから「比べて並べる人だけに効く」上の壁（下限）に縛られず、O(n log n) を突破できます
（徒競走の世界記録が自転車には関係ないのと同じイメージ）。
