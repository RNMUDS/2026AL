...
## 導入: 前回の振り返り — 「倍々に広げる探索」を日常で見つけてみよう

前回（第5回）の最後に、**指数探索** が「データ数が未知でも、target の位置に応じて自動でスピード調整できる」優れた発想であることを学びました。実はこの **「ゴールまでの距離が未知のとき、近場から倍々に範囲を広げる」** という考え方は、コンピュータの中だけでなく、私たちの日常生活でも自然に使われています。

### 身近な例をいくつか挙げてみると…

- 鍵をなくしたとき、まず **カバン → 机の周り → 部屋全体 → 家全体 → 家の外** … と探す範囲を広げていく
- 連絡が来ない友達を **「あと 5 分 → ダメならあと 10 分 → 20 分 → 1 時間」** と倍々に待つ
- PC の不調が「いつから？」を **「昨日 → 3 日前 → 1 週間前 → 1 ヶ月前」** と倍々に遡って思い出す
- ネットがつながらないとき、アプリが **「1 秒後 → 2 秒後 → 4 秒後 → 8 秒後」** と再接続を試みる（**指数バックオフ** という正式名称がついた標準テクニック）

これらはすべて **「線形に広げると遠いゴールに弱く、最初から最大範囲を全部探すと近いゴールに無駄が出る」** という問題に対し、**倍々** で両方のバランスを取る賢い戦略です。

### 考えてみよう（個人 → ペア → 全体共有 / 約5分）

次の3つの問いに答え、ペアで共有してみよう。

**Q1.** 自分の生活・バイト・趣味・勉強の中で、「**距離・量・時間が未知なとき、無意識に倍々で広げて探している / 試している**」場面を **1 つ** 挙げてみよう。

**Q2.** その場面で、もし **「線形に少しずつ広げる」** やり方をしたらどんな不便が生じるか説明してみよう。
（例: 鍵を探すのに「半径 10cm → 20cm → 30cm…」とゆっくり広げたら？）

**Q3.** 逆に **「最初から最大範囲を全部探す」** とどんな無駄が生じるか説明してみよう。
（例: 鍵を探すのに「いきなり半径 1km を全部しらみつぶし」したら？）

> 💡 **メタな視点**: アルゴリズムは「コンピュータの中だけの話」ではなく、**問題解決の型** として日常にも応用できます。今回学ぶ **ハッシュ法** も同じで、「ロッカー番号で一発検索」という日常的な経験を、データ構造として形にしたものです。前回の「探す範囲を賢く絞る」発想から、今回は「**そもそも探さずに一発で当てる**」発想にギアチェンジしていきます。
