...
#### 違い⑤ — なぜ「倍々」なのか？100 や 1000 ずつではダメな理由

「だったら 100 や 1000 ずつ範囲を広げてもいいのでは？」と思うかもしれません。しかし **固定刻みのジャンプ** には次の問題があり、**倍々（指数）** にすることでそれが解消されます。

**① target の位置に応じて自動で「ちょうどよい」スピードになる**

倍々に広げると、target がどこにいても **必要な分だけ** しか広げません。

| target の位置 | 倍々（指数）で到達するまで | 固定 100 で到達するまで | 固定 1000 で到達するまで |
|---|---|---|---|
| 5 番目 | 3 回（1→2→4→8） | 1 回 | 1 回 |
| 500 番目 | 10 回（…→1024） | 5 回 | 1 回 |
| 50,000 番目 | 17 回（…→131072） | **500 回** | 50 回 |
| 5,000,000 番目 | 23 回 | **50,000 回** | **5,000 回** |

固定刻みは「target が遠いほどジャンプ回数が **線形に増える**」のに対して、倍々は **log で増えるだけ**。これが O(log n) と O(n/step) の決定的な差です。

**② 「最適な step を事前に決められない」問題を回避できる**

固定刻みで最適な step を選ぼうとすると、

- step が小さい（例: 100） → target が遠いと回数爆発
- step が大きい（例: 10000） → target が近いと無駄に飛ばしすぎる
- 数学的な最適は `step = √n` → これがジャンプ探索だが、**n を知っている必要がある**

データ数 n が未知なら、そもそも「100 が良いか 1000 が良いか」を決める根拠がありません。倍々は **n を知らなくても自動的に target の位置に合わせて止まる** ので、この選択問題そのものが消えます。

**③ 「広げすぎた量」が target の位置以下に収まる**

倍々で止まった時点で `bound ≤ 2 × target の位置`。つまり

```
target が位置 k にあるとき、bound は最大でも 2k 程度で止まる
              ↓
二分探索する範囲 [bound/2, bound] の幅 ≤ k
              ↓
二分探索は log(k) 回で完了 → 全体で O(log k)
```

固定 1000 で広げると、target が位置 5 でも範囲 [0, 1000] を二分探索することになり、target に対して **過剰に広い範囲** を毎回二分探索する羽目になります。倍々ならこの「広げすぎ」が target に比例した量に抑えられます。

**④ どんな n でも自動でスケールする**

10 要素のリストでも 100 億要素のリストでも、同じコードがそのまま動きます。固定刻みだとデータ規模ごとに step を調整しないと性能が崩れます。

**直感的なまとめ**

固定刻みは **「一定速度で歩いて探す」**、倍々は **「歩幅をだんだん広げて走る」** イメージです。target が遠ければ遠いほど大股になり、近ければ近いだけ小股のまま止まる ── target との距離に応じて勝手にギアチェンジしてくれる、これが指数的に広げる本質的な利点です。

- 固定刻み s での到達回数: `target位置 / s` （**線形**）
- 倍々での到達回数: `log2(target位置)` （**対数**）

n が大きくなるほど、この差は決定的になります（n=10億 なら 線形 1000万回 vs 対数 30 回）。
