...
{fig_why_settle()}
    ...
    <div class="note-warn">
      ...
      「遠回りをすると時間が減る」ことが起きてしまうためです。後期の路線図や迷路では、時間やコストがマイナスになることはないので安心して使えます。
    </div>"""
...
{example_pair('AL2-05-ex1.py', '手順1で新宿を確定させると、渋谷7分・池袋9分・品川30分が書き込まれます。'
...
{example_pair('AL2-05-ex2.py', '各駅への最短の行き方が、駅名を矢印でつないだ形で表示されました。'
...
{setup_guide('05', ['AL2-05-ex1.py', 'AL2-05-ex2.py'])}
...
ans = answers([slide_example("05"), blank_answers("05"),
    ("確かめ用の数値", """        <p><strong>問い1（渋谷を出発点にしたとき）</strong></p>
        <table>
          ...
          <tr><td>時間</td><td>0分</td><td>7分</td><td>9分</td><td>16分</td><td>20分</td><td>26分</td></tr>
        </table>
        <p style="margin-top:0.6rem">確定していく順番は 渋谷 → 新宿 → 品川 → 池袋 → 東京 → 上野 です。
        上野はいったん28分と書かれたあと、東京を確定したときに<strong>26分に書き直されます</strong>。
        ここを見落とす人が多いので注意してください。</p>
        <p style="margin-top:0.8rem"><strong>問い2の根拠</strong>: ダイクストラ法はA店を1円で確定し、
        そのあとB店（2円）を調べたときに見つかる -3円 を「A店は確定済み」として無視します。</p>"""),
])
body = "\n".join([
    sub,
    section("sec-explanation", "1", "ダイクストラ法の考え方", explanation),
    section("sec-examples", "2", "例題", examples),
    slides_for("05", SLIDES),
    advanced_section("05"),
    rubric_section("05"),
    ans,
])
